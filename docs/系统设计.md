...
flowchart LR
  ...
  Frontend --> Generate["POST /api/scripts/generate"]
  Frontend --> Validate["POST /api/scripts/validate"]
  Generate --> Parser["章节解析"]
...
- 生成流程：提交标题、正文、输出格式、模型 id 和输出语言到后端。
...
- API 层：提供健康检查、AI 状态、模型列表、生成剧本和校验 YAML 接口。
...
3. 前端调用 `POST /api/scripts/generate`。
4. 后端校验请求体和章节数量。
5. 后端生成 YAML 骨架。
6. 如果使用 `local`，直接返回骨架；如果使用远程模型，则调用 Provider 改编骨架。
7. 远程 Provider 返回后，后端提取 YAML 并执行 Schema 校验。
8. 首轮远程输出未通过校验时，后端追加一次修复请求；仍失败则返回明确错误。
9. 前端展示 YAML 和结构化预览。
10. 用户进行结构化编辑或 YAML 编辑后，校验、复制和下载都读取同一份最新 YAML。
