...
- 生成流程：创建后台生成任务，使用 SSE 展示阶段估算进度、模型和耗时，断线时回退查询任务状态。
...
8. 如果使用 `local`，直接返回骨架；如果使用远程模型，则调用 Provider 改编骨架。
9. 远程 Provider 返回后，后端提取 YAML 并执行 Schema 校验。
10. 首轮远程输出未通过校验时，后端追加一次修复请求；仍失败则任务进入 `failed`。
11. SSE 断开时，前端回退调用 `GET /api/scripts/generate/jobs/{job_id}` 拉取最终状态。
12. 任务完成后，前端用最终 YAML 替换骨架预览，并刷新结构化预览。
13. 用户进行结构化编辑或 YAML 编辑后，校验、复制和下载都读取同一份最新 YAML。
...
- 长任务可见：长文本生成期间应持续展示阶段估算进度、骨架预览和最终结果。
