...
- 生成流程：创建后台生成任务，使用 SSE 展示进度、模型、耗时和远程模型流式 YAML，断线时回退查询任务状态。
...
- AI Provider：默认 `local` 返回骨架；远程模型通过 OpenAI-compatible Chat Completions 调用，并优先使用 streaming 响应。
...
7. 进入 `ai_generating` 阶段时，后端把 YAML 骨架作为 `preview_yaml` 推送给前端，前端先更新 YAML 面板和结构化预览。
8. 如果使用 `local`，直接返回骨架；如果使用远程模型，则以 `stream: true` 调用 Provider 改编骨架。
9. 远程 Provider 每返回一段 delta，后端累积为 `stream_yaml` 并通过 `job.status` SSE 推给前端。
10. 前端用 `stream_yaml` 持续更新 YAML 面板，并在当前片段可解析时刷新结构化预览。
11. 远程 Provider 返回完整内容后，后端提取 YAML 并执行 Schema 校验。
12. 首轮远程输出未通过校验时，后端追加一次修复请求；仍失败则任务进入 `failed`。
13. SSE 断开时，前端回退调用 `GET /api/scripts/generate/jobs/{job_id}` 拉取最终状态。
14. 任务完成后，前端用最终规范化 YAML 替换流式内容，并刷新结构化预览。
15. 用户进行结构化编辑或 YAML 编辑后，校验、复制和下载都读取同一份最新 YAML。
...
- 长任务可见：长文本生成期间应持续展示阶段进度、模型耗时、远程流式 YAML、已输出字符数和最终结果；骨架预览作为本地生成和失败回退能力保留。
