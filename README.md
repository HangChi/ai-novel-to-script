...
- 结构化预览与局部编辑：将当前 YAML 映射为结构化视图，支持编辑剧本标题和 logline，并只读展示人物、场景和 beats。
...
本地启动前后端后，可在前端导入示例文本，点击“生成 YAML”，查看 YAML 与结构化视图，编辑剧本标题和 logline，再使用“校验 YAML”“复制 YAML”或“下载 YAML”验证完整流程。
...
5. 修改剧本标题和 logline，确认 YAML 内容同步更新。
...
当前仓库已建立文档、前端、后端、SQL 与部署配置目录，并搭建了基础前后端框架、章节解析、剧本 YAML 初稿骨架、生成接口、YAML 校验接口、AI Provider 和前端 YAML 预览、结构化预览、标题/logline 结构化编辑、YAML 编辑、校验、复制、下载、本地文本导入能力。后续会逐步补充场景、人物、beats 的结构化编辑、双向同步和持久化能力。
