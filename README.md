...
- 结构化预览：将当前 YAML 映射为只读视图，展示标题、logline、人物、场景和 beats。
...
本地启动前后端后，可在前端导入示例文本，点击“生成 YAML”，查看 YAML 与结构化只读预览，再使用“校验 YAML”“复制 YAML”或“下载 YAML”验证完整流程。
...
3. 点击“生成 YAML”。
4. 查看右侧“结构化预览”，确认标题、logline、人物、场景和 beats 已从 YAML 映射为只读视图。
5. 点击“校验 YAML”，确认显示“YAML 结构有效”。
6. 点击“复制 YAML”或“下载 YAML”，验证当前剧本内容可以导出。
...
当前仓库已建立文档、前端、后端、SQL 与部署配置目录，并搭建了基础前后端框架、章节解析、剧本 YAML 初稿骨架、生成接口、YAML 校验接口、AI Provider 和前端 YAML 预览、结构化只读预览、编辑、校验、复制、下载、本地文本导入能力。后续会逐步补充结构化编辑、双向同步和持久化能力。
